...
- An `EventType` field is injected into each JSON record (`hail`, `wind`, or `tornado`)
...
- **500-599 errors**: Retry with fixed 5-minute interval (up to 3 retries, 4 attempts total)
...
- **Transient failure**: Retry with exponential backoff (1s, 2s) up to 3 attempts
...
```
CSV Fetch ─┐
           ├─ HTTP Retry Loop (5min × 4 attempts) ─┐
           │                                        ├─ Parse CSV → Kafka Publish (exp backoff × 3 attempts)
           │                                        └─ On 5xx: retry outer loop
           └─ Log Result
```
...
Cron scheduling is configured via the `CRON_SCHEDULE` environment variable (see `src/config.ts`).
...
NOAA's filename abbreviation `torn` is normalized to `tornado` in the published JSON. An `EventType` field is injected into each record.
...
HTTP fetches use fixed 5-minute interval retry (up to 3 retries, 4 attempts total) for 5xx errors. Kafka publishing uses exponential backoff (1s, 2s) up to 3 attempts for transient failures.
